...
#    @reserva.build_monto
  ...
  def to_csv(relation)
    ...
    CSV.generate(:col_sep => ";") do |csv|
      csv << %w[ id agencia operadora salida referencia programa periodo regimen titular pasajeros bruto seguro iva neto moneda ]
      relation.each do |r|
        csv << %W[ #{r.id} #{r.agencia.name} #{r.operadora.name} #{r.salida} #{r.referencia} #{r.programa.try(:name)} #{r.periodo} #{r.regimen} #{r.titular} #{r.pasajeros.size} #{r.tarifa.try(:localize)} #{r.seguro.try(:localize)} #{r.iva.try(:localize)} #{r.total} #{r.total_currency}]
      end
    end
  end
end
