class PasajeroReport < Prawn::Document
  ...
  def to_pdf(object)    
    ...
    repeat :all do
      font_size 10 do 
        ...
        draw_text "Listado de pasajeros", :at => [0,710]
        stroke do
      ...
      font_size 6 do
        ...
        draw_text "Litado de pasajeros",:at => [100,5]
        number_pages "<page> de <total>", [bounds.right - 50, 5] 
        draw_text "power by Blue Lemons",:at => [bounds.right - 150, 5] 
      end       
    end
    
    myrow = [["ID"",doc" ",name" ",nacimiento" ",tdoc_id" ]]
    object.each do |r|
      myrow += [["#{r.id}"",#{r.doc}" ",#{r.name}" ",#{r.nacimiento}" ",#{r.tdoc_id}" ]]
    end
