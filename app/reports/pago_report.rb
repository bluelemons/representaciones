class PagoReport < Prawn::Document
  ...
  def to_pdf(object)    
    ...
    repeat :all do
      font_size 10 do 
        ...
        draw_text "Listado de pagos", :at => [0,710]
        stroke do
      ...
      font_size 6 do
        ...
        draw_text "Litado de pagos",:at => [100,5]
        number_pages "<page> de <total>", [bounds.right - 50, 5] 
        draw_text "power by Blue Lemons",:at => [bounds.right - 150, 5] 
      end       
    end
    
    myrow = [["ID"",fecha" ",monto" ",moneda_id" ]]
    object.each do |r|
      myrow += [["#{r.id}"",#{r.fecha}" ",#{r.monto}" ",#{r.moneda_id}" ]]
    end
