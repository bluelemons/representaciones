class OperadoraReport < Prawn::Document
  ...
  def to_pdf(object)    
    ...
    repeat :all do
      font_size 10 do 
        ...
        draw_text "Listado de operadoras", :at => [0,710]
        stroke do
      ...
      font_size 6 do
        ...
        draw_text "Litado de operadoras",:at => [100,5]
        number_pages "<page> de <total>", [bounds.right - 50, 5] 
        draw_text "power by Blue Lemons",:at => [bounds.right - 150, 5] 
      end       
    end
    
    myrow = [["ID"",name" ",direccion_id" ]]
    object.each do |r|
      myrow += [["#{r.id}"",#{r.name}" ",#{r.direccion_id}" ]]
    end
