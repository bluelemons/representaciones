class TarifaReport < Prawn::Document
  ...
  def to_pdf(object)    
    ...
    repeat :all do
      font_size 10 do 
        ...
        draw_text "Listado de tarifas", :at => [0,710]
        stroke do
      ...
      font_size 6 do
        ...
        draw_text "Litado de tarifas",:at => [100,5]
        number_pages "<page> de <total>", [bounds.right - 50, 5] 
        draw_text "power by Blue Lemons",:at => [bounds.right - 150, 5] 
      end       
    end
    
    myrow = [["ID"",iva" ",impuesto" ",monto" ",comision" ",efectivo" ",anticipo" ",comision" ",obs" ",reserva_id" ]]
    object.each do |r|
      myrow += [["#{r.id}"",#{r.iva}" ",#{r.impuesto}" ",#{r.monto}" ",#{r.comision}" ",#{r.efectivo}" ",#{r.anticipo}" ",#{r.comision}" ",#{r.obs}" ",#{r.reserva_id}" ]]
    end
