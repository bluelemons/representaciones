class <%= file_name_c %>Report < Prawn::Document
  ...
  def to_pdf(object)    
    ...
    repeat :all do
      font_size 10 do 
        ...
        draw_text "Listado de <%= plural_name %>", :at => [0,710]
        stroke do
      ...
      font_size 6 do
        ...
        draw_text "Litado de <%= plural_name %>",:at => [100,5]
        number_pages "<page> de <total>", [bounds.right - 50, 5] 
        draw_text "power by Blue Lemons",:at => [bounds.right - 150, 5] 
      end       
    end
    
    myrow = [["ID"<%for attribute in @model_attributes %>",<%= attribute.name %>" <% end %>]]
    object.each do |r|
      myrow += [["#{r.id}"<%for attribute in @model_attributes %>",#{r.<%= attribute.name %>}" <% end %>]]
    end
